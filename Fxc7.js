...
mimetype: 'video/mp4', filename: `Imlexa.mp4`, quoted: freply
  })
...
const duit = checkATMuser(sender)
					await costum(help(prefix, instagram, getLevelingLevel, getLevelingXp, sender, name, pushname2, user, limitt, uptime, jam, tanggal, duit), text, FarhanGans, freply)
    				break 
    			case 'infobot':
			...
			if (isBanned) return reply(mess.only.benned)    
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						Zitsraa.sendMessage(from, buffer, image, {quoted: freply, caption: 'nihhh'})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender) 
					break 
...
case 'leaderboard':
		...
		case 'jsholat':
				...
				if (isLimit(sender)) return reply(limitend(pushname2))
					...
					sholat = `${body.slice(9)}`
					anu = await fetchJson(`https://api.haipbis.xyz/jadwalsholat?daerah=${sholat}`, {method: 'get'})
					reply(mess.wait)
...
case 'xxx':
			   if (isBanned) return reply(mess.only.benned)    
			   if (!isUser) return reply(mess.only.userB)
			   if (!isPremtod) return reply('premiumin tod!')
			   if (isLimit(sender)) return reply(limits.limitend(pushname2))
			   reply(mess.wait)
              	    if (args.length < 1) return reply('teksnya mana gan?')
                    teks = body.slice(5)
...
// premium user
   case 'joox':
  if (!isUser) return reply(mess.only.userB)
  if (!isPremtod) return reply('Mode premium Aktifin Ngab')
